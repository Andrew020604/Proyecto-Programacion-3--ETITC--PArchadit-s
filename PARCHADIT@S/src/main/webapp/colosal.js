...
class Evento {
    ...
    constructor(Titulo, Categoria, Ubicacion, Descripcion, CuposTotales, CreadoPor) {
        ...
        this.CuposTotales = Number(CuposTotales);
        ...
        usuariosPlanos.forEach(function (usuarioPlano) {
            ...
            const usuarioReconstruido =
                new Usuario(
                    usuarioPlano.nombre,
                    usuarioPlano.correo,
                    usuarioPlano.contraseña
                );

            usuarioReconstruido.intereses =
                usuarioPlano.intereses || [];
...
function cargarEventos() {
    ...
    if (datosGuardados) {
        ...
        eventosPlanos.forEach(function (eventoPlano) {
            ...
            eventoReconstruido.CuposOcupados =
                eventoPlano.CuposOcupados || 0;

            eventoReconstruido.likes =
                eventoPlano.likes || 0;

            eventoReconstruido.comentarios =
                eventoPlano.comentarios || [];

            eventoReconstruido.likesUsuarios =
                eventoPlano.likesUsuarios || [];

            eventoReconstruido.asistentes =
                eventoPlano.asistentes || [];

            eventoReconstruido.CreadoPor =
                eventoPlano.CreadoPor || null;
...
function renderFeed() {
    ...
    if (modofeed === "para ti" && usuarioActual) {
        ...
        eventosBase =
            eventos.filter(function (evento) {
                ...
                return usuarioActual.intereses.includes(
                    evento.Categoria
                );
            });
    }
    ...
    eventosFiltrados.forEach(function (evento) {
      ...
      <article class="card">
        ...
        <p class="card-meta">
          📍 ${evento.Ubicacion} · ${evento.Categoria}
        </p>
        ...
        <p class="card-cupos">
          Total de cupos para completar el parche:
          ${evento.CuposTotales}
        </p>
...
feedContainer.addEventListener('click', function (e) {
    ...
    if (e.target.closest('.btn-like')) {
        ...
        if (!sePudo) {
            ...
            mostrarToast(
                'Ya le diste like a este evento.',
                'error'
            );
        }
    ...
    if (e.target.closest('.btn-unirse')) {
        ...
        if (resultado === true) {
            ...
            mostrarToast(
                '¡Quedaste en el parche! 🎉'
            );

        } else if (resultado === 'ya_unido') {

            mostrarToast(
                'Ya estás en este parche.',
                'error'
            );
...
btnCerrarSesion.addEventListener('click', function () {
    ...
    btnExplorar.classList.add(
        'activar-feed'
    );

    btnParaTi.classList.remove(
        'activar-feed'
    );
...
const campoNombre =
    document.getElementById('aNombre');

const btnAuthSubmit =
    document.getElementById('btnAuthSubmit');

const formAuth =
    document.getElementById('formAuth');

const camposIntereses =
    document.getElementById('camposIntereses');
...
// ============================================================
// ENVIAR LOGIN / REGISTRO AL SERVLET
// ============================================================
...
formAuth.addEventListener('submit', function (e) {
    ...
    // IMPORTANTE:
    // El ID correcto en index.html es aContrasena
    const contraseña =
        document.getElementById('aContrasena').value;
...
// ============================================================
// CREAR EVENTO - VENTANA
// ============================================================

const overlayCrear =
    document.getElementById('overlayCrear');

const btnCrearEvento =
    document.getElementById('btnCrearEvento');

const btnCerrarCrear =
    document.getElementById('btnCerrarCrear');


overlayCrear.style.display =
    'none';
